<h1 align="center">Hi 👋, I'm Nitika</h1>

<h3 align="center">
🚀 Full Stack Developer | Open Source Contributor | Backend Enthusiast
</h3>

<p align="center">
Passionate about building impactful software, solving real-world problems through code, and continuously growing as a developer.
Currently focused on Full Stack Development, Backend Engineering, DSA, and Open Source Contributions.
</p>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Niti1203&label=Profile%20Views&color=0e75b6&style=flat" alt="profile views" />
</p>

---

## 🌱 Currently Learning & Building

- Data Structures & Algorithms in Java & C++
- Backend Development with Spring Boot
- REST APIs & Database Management
- System Design Fundamentals
- Open Source Collaboration & Git Workflows
- Scalable Full Stack Applications

---

## 💻 Tech Stack

### Languages
C++ • Java • Python • JavaScript • C • HTML • CSS

### Frameworks & Technologies
React • Spring Boot • Tailwind CSS • Bootstrap • MongoDB • MySQL

### Tools & Platforms
Git • GitHub • Postman • Figma • IntelliJ IDEA • VS Code

### Other Skills
UI/UX Design • API Integration • Problem Solving • Team Leadership • Project Management

---

## 🚀 Current Projects

### 🛒 SmartCart
A Full Stack E-Commerce platform focused on scalable backend systems and modern user experience.

#### Features Worked On
- Authentication & Authorization
- REST API Integration
- MySQL Database Handling
- Backend Development using Spring Boot
- API Testing with Postman
- Frontend & Backend Integration

---

## 🌟 Open Source Journey

- Contributing to real-world GitHub repositories
- Working with collaborative development workflows
- Learning production-level Git & GitHub practices
- Exploring backend architecture and feature implementation
- Improving code quality through real project contributions

---

## 🏆 Achievements & Experience

- Contributed to open-source projects on GitHub
- Building real-world full stack applications using Spring Boot & React
- Solved DSA and problem-solving questions regularly on LeetCode
- Participated in multiple hackathons and technical competitions
- Led teams and coordinated technical & gaming events during college fests
- Built Arduino-based engineering and hardware projects
- Experienced with UI/UX design workflows using Figma
- Continuously learning backend systems, APIs, and scalable development practices
- Strong interest in product building, startups, and impactful tech solutions

---

## 📈 LeetCode Progress

<p align="center">
  <img src="https://leetcard.jacoblin.cool/whytiskolaveri12?theme=light&font=baloo&ext=contest" />
</p>

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Niti1203&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Niti1203&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Niti1203&layout=compact&theme=tokyonight" />
</p>

---

## 🏅 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Niti1203&theme=radical&row=2&column=4&margin-w=15&margin-h=15&no-frame=true" />
</p>

---

## 📫 Connect With Me

- 📧 Email: **nitiwork90@gmail.com**
- 💼 LinkedIn: https://www.linkedin.com/in/nitika-0245b6251/
- 🧠 LeetCode: https://leetcode.com/u/whytiskolaveri12/
- 📸 Instagram: https://instagram.com/whytiskolaveri

---

## ⚡ Fun Fact

I enjoy turning ambitious ideas into real projects and constantly pushing myself to learn new technologies and improve every single day.

---

<h3 align="center">
⭐ Always Building • Always Learning • Always Improving
</h3>
